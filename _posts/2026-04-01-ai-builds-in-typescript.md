...
Then in 2009, Ryan Dahl created [node.js](https://nodejs.org/), a stand-alone executable that can JavaScript outside the confines of the browser. More importantly, it provided a brand-new, non-standard "platform API" giving JavaScript access to the filesystem and the network. This flexibility, along with node's embrace of CommonJS modules and the rise of `npm` contributed significantly to the "API economy" and made "server-side JavaScript" a preferred choice for microservices and web app servers.

Fast-forward to late 2025 and the appearence of the earliest "agent AIs" such as OpenClaw and Claude Code, and we are starting to see JavaScript emerge once again as a target to execute little scripts, only this time the script code is being auto-generated by an LLM, and the host API it can use is even less well-specified - a loose collection of "tools" giving the code access to potentially every app, document and internet account on your computer.
